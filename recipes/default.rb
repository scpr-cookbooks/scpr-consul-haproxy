...
admin_ip = node.network.interfaces[ node.scpr_consul_haproxy.admin_interface ].addresses.find do |_k, v|
  v.family == 'inet'
end.first

# Install SSL certificates
if certs = begin
             node.ssl.databag_items
           rescue
             nil
           end
  certs.each do |cert_databag_item|
    next unless cert = begin
                         data_bag_item(node.ssl.databag, cert_databag_item)
                       rescue
                         nil
                       end
